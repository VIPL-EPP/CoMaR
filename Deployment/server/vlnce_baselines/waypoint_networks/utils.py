...
def get_heading_robust(agent_state):
    q = agent_state['rotation']
    
    # 1. 定义局部坐标系下的正前方向量 (Habitat/OpenGL 中 -Z 是前方)
    forward_local = np.array([0.0, 0.0, -1.0])
    
    # 2. 使用四元数旋转该向量，得到世界坐标系下的朝向
    # numpy-quaternion 库提供了 rotate_vectors 方法
    forward_world = quaternion.rotate_vectors(q, forward_local)
    
    # 3. 在 X-Z 平面上计算角度 (忽略 Y 轴高度)
    # arctan2(y, x) -> 在这里对应 arctan2(world_x, -world_z)
    # 解释：我们通常希望 -Z (前方) 对应 0 度，或者根据你的坐标系定义调整 x, z 的顺序
    # 你的原代码逻辑似乎想要标准的 2D 平面角
    o = np.arctan2(forward_world[0], -forward_world[2])
...
def get_sim_location(agent_state):
    ...
    if (axis%(2*np.pi)) < 0.05 or (axis%(2*np.pi)) > 2*np.pi - 0.05:
        o = quaternion.as_euler_angles(agent_state['rotation'])[1]
    else:
        o = 2*np.pi - quaternion.as_euler_angles(agent_state['rotation'])[1]
    if o > np.pi:
        o -= 2 * np.pi
    # o = get_heading_robust(agent_state)
    pose = x, y, o
    return pose, height
...
def depth_to_3D(depth_obs, img_size, xs, ys, inv_K):
    ...
    xys = torch.vstack((torch.mul(xs, depth) , torch.mul(ys, depth), -depth, torch.ones(depth.shape, device=depth_obs.device))) # 4 x 128 x 128
    xys = xys.reshape(4, -1)
    xy_c0 = torch.matmul(inv_K, xys)

    # SPEEDUP - don't allocate new memory, manipulate existing shapes
    local3D = torch.zeros((xy_c0.shape[1],3), dtype=torch.float32, device=depth_obs.device)
    local3D[:,0] = xy_c0[0,:]
    local3D[:,1] = xy_c0[1,:]
    local3D[:,2] = xy_c0[2,:]
