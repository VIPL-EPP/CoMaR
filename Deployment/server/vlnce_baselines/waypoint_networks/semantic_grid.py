...
class SemanticGrid(object):
    ...
    def __init__(self, batch_size, grid_dim, crop_size, cell_size, spatial_labels, object_labels):
        ...
        self.occupancy_grid = torch.ones((self.batch_size, self.spatial_labels, self.grid_dim[0], self.grid_dim[1]), dtype=torch.float16, device=self.device)
        self.occupancy_grid = self.occupancy_grid*(1/self.spatial_labels)

        self.semantic_grid = torch.ones((self.batch_size, self.object_labels, self.grid_dim[0], self.grid_dim[1]), dtype=torch.float16, device=self.device)
    ...
    def spatialTransformer(self, grid, pose, abs_pose):
        ...
        geo_grid_out = torch.zeros((grid.shape[0], grid.shape[1], self.grid_dim[0], self.grid_dim[1]), dtype=torch.float16).to(grid.device)
        ...
        for j in range(grid.shape[0]): # sequence length
            ...
            init_rot_mat = torch.tensor([[torch.cos(init_pose[2]), -torch.sin(init_pose[2])],
                                        [torch.sin(init_pose[2]),torch.cos(init_pose[2])]], dtype=torch.float16).to(grid.device)

            grid_step = grid[j,:,:,:].unsqueeze(0)
            pose_step = pose[j,:]
        
            rel_coord = torch.tensor([pose_step[1],pose_step[0]], dtype=torch.float16).to(grid.device)
            ...
            trans_theta = torch.tensor( [ [1, -0, x], [0, 1, z] ], dtype=torch.float16 ).unsqueeze(0)
            rot_theta = torch.tensor( [ [torch.cos(angle), -1.0*torch.sin(angle), 0], [torch.sin(angle), torch.cos(angle), 0] ], dtype=torch.float16 ).unsqueeze(0)
    ...
    def rotate_map(self, grid, rel_pose, abs_pose):
            ...
            ego_grid_out = torch.zeros((grid.shape[0], grid.shape[1], self.grid_dim[0], self.grid_dim[1]), dtype=torch.float16).to(grid.device)
            ...
            for i in range(grid.shape[0]): # sequence length
                ...
                rel_coord = torch.tensor([rel_pose_step[1],rel_pose_step[0]], dtype=torch.float16).to(grid.device)
                rel_coord = rel_coord.reshape((2,1))
                rel_coord = torch.matmul(init_rot_mat,rel_coord)
                x = -2*(rel_coord[0]/self.cell_size)/(self.grid_dim[0])
                z = -2*(rel_coord[1]/self.cell_size)/(self.grid_dim[1])
                ...
                trans_disp_grid = F.affine_grid(trans_theta, grid_step.size(), align_corners=False) # get grid translation displacement
                rot_disp_grid = F.affine_grid(rot_theta, grid_step.size(), align_corners=False) # get grid rotation displacement
                trans_ego_grid = F.grid_sample(grid_step, trans_disp_grid.to(torch.float16), align_corners=False ) # apply translation 
                ego_grid = F.grid_sample(trans_ego_grid, rot_disp_grid.to(torch.float16), align_corners=False) # apply rotation
    ...
    def update_proj_grid_bayes(self, occup_grid=None, segm_grid=None):
        ...
        step_occup_grid = torch.zeros((occup_grid.shape[0], occup_grid.shape[1], self.spatial_labels, 
                                            self.grid_dim[0], self.grid_dim[1]), dtype=torch.float16).to(occup_grid.device)
        ...
        step_segm_grid = torch.zeros((segm_grid.shape[0], segm_grid.shape[1], self.object_labels, 
                                            self.grid_dim[0], self.grid_dim[1]), dtype=torch.float16).to(segm_grid.device)
