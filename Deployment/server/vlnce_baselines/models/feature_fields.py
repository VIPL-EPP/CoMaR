...
class Feature_Fields(nn.Module):
    ...
    def run_region_encode(self, sample_ft_neighbor_embedding, sample_ft_neighbor_xyzds):
        ...
        sample_input = self.fcd_aggregation(sample_ft_neighbor_embedding.to(torch.float32)+sample_ft_neighbor_xyzds.to(torch.float32))
    ...
    def get_bev_visual_target(self, batch_target_depth, batch_target_bbox, batch_target_camera_intrinsic, batch_target_rot, batch_target_trans, batch_agent_rot, batch_agent_trans, depth_scale=1000.0, depth_trunc=1000.0):
        # batch_target_bbox: batch_size x num_of_bbox x [left_x,left_y,right_x,right_y]

        thread_output = self.thread_pool([ [project_depth_to_3d, [ batch_target_depth[job_id].clone(), batch_target_camera_intrinsic[job_id], batch_target_rot[job_id], batch_target_trans[job_id],depth_scale,depth_trunc,self.args.input_height,self.args.input_width ], {} ] for job_id in range(len(batch_target_depth)) ]) # Parallel computing with multiple CPUs

        batch_gt_label = []
        for batch_id in range(self.batch_size):    
            depth_map_shape = batch_target_depth[batch_id].shape
            left_x,left_y,right_x,right_y = batch_target_bbox[batch_id]
            left_x, right_x =  int(left_x * (self.args.input_height / depth_map_shape[0])), int(right_x * (self.args.input_height / depth_map_shape[0]))
            left_y, right_y =  int(left_y * (self.args.input_width / depth_map_shape[1])), int(right_y * (self.args.input_width / depth_map_shape[1]))
            points, points_mask = thread_output[batch_id]

            R = batch_target_rot[batch_id]
            T = batch_target_trans[batch_id]
            points = (R @ points.T + T).T  


            points = torch.tensor(points).view(self.args.input_height,self.args.input_width,3)
            bbox_points = points[left_x:right_x,left_y:right_y].to(self.device)

            bbox_points_tree = build_kd_tree(bbox_points.to(torch.float32))
            
            R = batch_agent_rot[batch_id]
            T = batch_agent_trans[batch_id]
            rel_position, rel_direction, rel_dist = self.bev_sampled_rays
            ray_xyz = (R @ rel_position.reshape((-1,3)).T + T).T
            ...
            with torch.no_grad():
                bbox_dists, bbox_inds = bbox_points_tree.query(ray_xyz.view(-1,3), nr_nns_searches=self.args.bev_feature_fields_search_num)
                bbox_dists = torch.sqrt(bbox_dists) # Note that the cupy_kdtree distances are squared

            bbox_dists = bbox_dists.view(self.args.localization_map_height,self.args.localization_map_width,self.args.bev_N_samples,self.args.bev_feature_fields_search_num).sum(-1).sum(-1)
            bbox_dists = bbox_dists.view(1,1,self.args.localization_map_height, self.args.localization_map_width)
            kernel_size = self.args.localization_map_height // self.args.grid_map_height
            bev_label = nn.functional.avg_pool2d(bbox_dists,kernel_size,kernel_size).view(-1)
            _, bev_label = bev_label.min(dim=0)
            batch_gt_label.append(bev_label.cpu().numpy().item())

        return batch_gt_label
