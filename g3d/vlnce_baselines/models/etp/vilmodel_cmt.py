...
class Lookahead_ImageEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LocalVPEncoder(nn.Module):
    def __init__(self, config):
        ...
        #)
        scale = 768 ** -0.5
        #self.cand_id_embeddings = nn.Parameter(scale * torch.randn(4,1,768))
...
class GlobalMapEncoder(nn.Module):
    ...
    def forward(
        ...
        gmap_embeds = self.encoder(
            txt_embeds, txt_masks, gmap_embeds, gmap_masks, batch_cand_panorama_fts, batch_selected_cand_ids,
            graph_sprels=graph_sprels
        )
        return gmap_embeds
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.lookahead_img_embeddings = Lookahead_ImageEmbeddings(config)

        self.local_encoder = LocalVPEncoder(config)
        self.local_sap_head = NextActionPrediction(self.config.hidden_size, 0.1)
        ...
        for param in self.lang_encoder.parameters():
            param.requires_grad = False
        for param in self.img_embeddings.parameters():
            param.requires_grad = False
        for param in self.global_encoder.parameters():
            param.requires_grad = False
        for param in self.lookahead_img_embeddings.parameters():
            param.requires_grad = False
        for param in self.local_encoder.parameters():
            param.requires_grad = False
        # for param in self.global_sap_head.parameters():
        #     param.requires_grad = False
        # for param in self.local_sap_head.parameters():
        #     param.requires_grad = False
    ...
    def forward_lookahead_panorama(
        self, rgb_fts, loc_fts
    ):
        device = rgb_fts.device

        rgb_embeds = self.lookahead_img_embeddings.img_layer_norm(
            self.lookahead_img_embeddings.img_linear(rgb_fts)
        )
        
        img_embeds = rgb_embeds

        pano_embeds = img_embeds + \
                      self.lookahead_img_embeddings.loc_layer_norm(self.lookahead_img_embeddings.loc_linear(loc_fts))

        pano_embeds = self.lookahead_img_embeddings.layer_norm(pano_embeds)
        pano_embeds = self.lookahead_img_embeddings.dropout(pano_embeds)

        if self.lookahead_img_embeddings.pano_encoder is not None:
            pano_embeds = self.lookahead_img_embeddings.pano_encoder(
                pano_embeds, src_key_padding_mask=None
            )
        return pano_embeds
    ...
    def forward_navigation(
        ...
        global_logits_org = global_logits.clone().detach()
        ...
        outs = {
            ...
            'global_logits_org': global_logits_org,
        }
        return outs


    def forward_lookahead_navigation(
        self, txt_embeds, txt_masks, 
        gmap_vpids, gmap_step_ids, 
        gmap_img_fts, gmap_pos_fts, 
        gmap_masks, gmap_visited_masks, gmap_pair_dists, cand_panorama_fts, selected_cand_ids, lookahead_loc_fts
    ):
        with torch.no_grad():
            # global branch
            gmap_step_embeds = self.global_encoder.gmap_step_embeddings(gmap_step_ids)
            gmap_pos_embeds = self.global_encoder.gmap_pos_embeddings(gmap_pos_fts)
            gmap_embeds = gmap_img_fts + gmap_step_embeds + gmap_pos_embeds
                          

        batch_size = cand_panorama_fts.shape[0]
        cand_num = cand_panorama_fts.shape[1]

        lookahead_logits = []
        lookahead_loc_fts = torch.cat([lookahead_loc_fts.unsqueeze(1) for cand_idex in range(cand_num)],dim=1)
        vp_input_embeds = self.forward_lookahead_panorama(cand_panorama_fts.reshape(batch_size,cand_num*12,768),lookahead_loc_fts.reshape(batch_size,cand_num*12,-1)).view(batch_size,cand_num,12,768)

        for batch_id in range(batch_size):
            vp_input_embeds[batch_id] = vp_input_embeds[batch_id] + gmap_step_embeds[batch_id][selected_cand_ids[batch_id]].unsqueeze(-2).repeat(1,12,1).detach() + gmap_pos_embeds[batch_id][selected_cand_ids[batch_id]].unsqueeze(-2).repeat(1,12,1).detach()

        vp_input_embeds = vp_input_embeds.view(batch_size,cand_num*12,768)

        #vp_input_embeds = (vp_input_embeds.view(batch_size,cand_num,12,768) + torch.cat([self.local_encoder.cand_id_embeddings]*12,dim=1)).view(batch_size,cand_num*12,768)
        vp_masks = torch.ones((batch_size,cand_num*12)).bool().to(vp_input_embeds.device)

        extended_vp_masks = extend_neg_masks(vp_masks)
        extended_txt_masks = extend_neg_masks(txt_masks)
        extended_gmap_masks = extend_neg_masks(gmap_masks)

        vp_embeds = torch.cat([vp_input_embeds,gmap_embeds.detach()],dim=1)
        extended_vp_masks = torch.cat([extended_vp_masks,extended_gmap_masks.detach()],dim=-1)

        for layer_module in self.local_encoder.encoder.x_layers:
            vp_embeds = layer_module(
                txt_embeds.detach(), extended_txt_masks.detach(),
                vp_embeds, extended_vp_masks
            )
        vp_embeds = vp_embeds[:,:vp_input_embeds.shape[1]].view(batch_size,cand_num,12,768)
        lookahead_logits = self.local_sap_head(vp_embeds).squeeze(-1)
        lookahead_logits = torch.cat([lookahead_logits[:,:,:4],lookahead_logits[:,:,-3:]],dim=-1)
        lookahead_logits, _ = torch.max(lookahead_logits,dim=-1)
        lookahead_logits = lookahead_logits.view(batch_size,cand_num)

        return lookahead_logits


    def forward(self, mode, batch, **kwargs):
        print("The forward function has been removed in GlocalTextPathNavCMT.")
        exit()