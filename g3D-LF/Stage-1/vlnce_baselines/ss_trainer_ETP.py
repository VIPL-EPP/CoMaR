    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        for stepk in range(self.max_len):
            ...
            for i in range(self.envs.num_envs):
                ...
                ghose_set = self.gmaps[i].update_graph(prev_vp[i], prev_ghost_emb[i], stepk+1,
                                           cur_vp[i], cur_pos[i], cur_embeds,
                                           cand_vp[i], cand_pos[i], cand_embeds,
                                           cand_real_pos[i])
                cur_path_ghost.append(ghose_set)
            ...
            path_score, probs, unvisited_or_current = self.policy.net(**CCM_inputs)
            ...
            # fused_logits = nav_logits#.detach().clone()

            # batch_selected_cand_ids = batch_selected_cand_ids.to(nav_logits.device)
            # for batch_id in range(nav_logits.shape[0]):
            #     fused_logits[batch_id][batch_selected_cand_ids[batch_id]] = lookahead_logits[batch_id]
            
            if mode == 'train' or self.config.VIDEO_OPTION:
                teacher_actions = self._teacher_action_new(nav_inputs['gmap_vp_ids'], no_vp_left)
            if mode == 'train':
                loss += F.cross_entropy(path_score, teacher_actions, reduction='sum', ignore_index=-100)
                # loss += F.cross_entropy(fused_logits, teacher_actions, reduction='sum', ignore_index=-100)
