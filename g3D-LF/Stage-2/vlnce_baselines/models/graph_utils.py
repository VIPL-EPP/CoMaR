...
def get_cand_heading(a, b, base_heading=0 ):
    # a, b: (x, y, z)
    dx = b[0] - a[0]
    dy = b[1] - a[1]
    dz = b[2] - a[2]
...
class GraphMap(object):
    def __init__(self, has_real_pos, loc_noise, merge_ghost, ghost_aug):
        ...
        self.Path_graph = nx.DiGraph()  # for path finding

        self.node_pos = {}          # viewpoint to position (x, y, z)
        self.node_embeds = {}       # viewpoint to pano feature
        self.node_stepId = {}
        self.org_node_id = None     # original node id
        self.node_path_embeds = {}  # viewpoint to path feature
        ...
        self.ghost_front_edge = {}  # ghost viewpoint to front viewpoint
        self.ghost_path_embeds = {}  # ghost viewpoint to path feature
    ...
    def delete_ghost(self, vp):
        ...
        self.ghost_front_edge.pop(vp)
        self.ghost_path_embeds.pop(vp)
        if self.has_real_pos:
            self.ghost_real_pos.pop(vp)

    def update_graph(self, prev_vp, prev_ghost_emb, step_id,
        ...
        self.Path_graph.add_node(cur_vp)
        if prev_vp is not None:
            ...
            self.Path_graph.add_edge(prev_vp, cur_vp, feature=prev_ghost_emb) 
        else:
            # if prev_vp is None, it is the first node
            self.org_node_id = cur_vp
        ...
        for i, (cvp, cpos, cembeds) in enumerate(zip(cand_vp, cand_pos, cand_embeds)):
            ...
            if localized_nvp is not None :
                ...
                self.Path_graph.add_edge(cur_vp, localized_nvp, feature=cembeds)
            # cand not overlap with node, create/update ghost
                ...
                if self.merge_ghost:
                    ...
                    if localized_gvp is None:
                        ...
                        self.ghost_front_edge[gvp] = {cur_vp : [cembeds, 1]}
                        ghost_set.add(gvp)
                        if self.has_real_pos:
                    ...
                    # update ghost
                        ...
                        if cur_vp not in self.ghost_front_edge[gvp]:
                            self.ghost_front_edge[gvp][cur_vp] = [cembeds, 1]
                        else:
                            self.ghost_front_edge[gvp][cur_vp][0] = self.ghost_front_edge[gvp][cur_vp][0] + cembeds
                            self.ghost_front_edge[gvp][cur_vp][1] += 1
                        ghost_set.add(gvp)
                        if self.has_real_pos:
        ...
        return list(ghost_set)

    def get_shortest_path(self, end_vp):
        return nx.shortest_path(self.Path_graph, self.org_node_id, end_vp)

    def get_ghost_front_edges(self, ghost, vp):
        return self.ghost_front_edge[ghost][vp][0] / self.ghost_front_edge[ghost][vp][1]
    
    def update_path_embeds(self, path_tokens, cur_path_vp, cur_path_ghost):
        self.node_path_embeds[cur_path_vp] = path_tokens[0]
        for i, gh in enumerate(cur_path_ghost):
            if gh in self.ghost_path_embeds.keys():
                self.ghost_path_embeds[gh][0] = self.ghost_path_embeds[gh][0] + path_tokens[i+1]
                self.ghost_path_embeds[gh][1] += 1
            else:
                self.ghost_path_embeds[gh] = [path_tokens[i+1], 1]

    def get_path_embeds(self, vp):
        if not vp.startswith('g'):
            return self.node_path_embeds[vp]
        else:
            return self.ghost_path_embeds[vp][0] / self.ghost_path_embeds[vp][1]
    ...
    def get_cand_pos(self, cur_vp, cur_pos, cur_ori, gmap_vp_ids):
        cand_positions, cand_headings, cand_dists = [], [], []
        cur_heading = heading_from_quaternion(cur_ori)
        for vp in gmap_vp_ids:
            if vp is None:
                cand_positions.append(cur_pos)
                cand_headings.append(cur_heading) # heading angle
                cand_dists.append(0.)
            # for ghost
            elif vp.startswith('g'):
                base_heading = heading_from_quaternion(cur_ori)
                base_elevation = 0
                vp_pos = self.ghost_aug_pos[vp]
                cand_heading = get_cand_heading(
                    cur_pos, vp_pos, base_heading
                )
                front_dis, front_vp = self.front_to_ghost_dist(vp)
                shortest_dist = self.shortest_dist[cur_vp][front_vp] + front_dis
                
                cand_positions.append(vp_pos)
                cand_headings.append(cand_heading%(2*np.pi))
                cand_dists.append(shortest_dist)

            # for node
            else:
                cand_positions.append([0., 0., 0.])
                cand_headings.append(0.) # heading angle
                cand_dists.append(float('inf'))

        cand_positions = np.array(cand_positions)
        cand_headings = np.array(cand_headings)
        cand_dists = np.array(cand_dists)
        return cand_positions, cand_headings, cand_dists