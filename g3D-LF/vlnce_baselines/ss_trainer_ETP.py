...
from vlnce_baselines.models.graph_utils import GraphMap, MAX_DIST, heading_from_quaternion
...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _set_config(self):
        ...
        if self.world_size > 1:
            ...
            self.local_rank = distr.get_rank()
    ...
    def _initialize_policy(
        ...
        ''' initialize the 3D Feature Fields here '''
        # Load the 3D Feature Fields model
        self.feature_fields = Feature_Fields(batch_size=1, device='cuda', mode='habitat',camera_intrinsic=None, bev=True)
        self.feature_fields.load_state_dict(torch.load("data/3dff.pth", map_location="cpu"))
        self.feature_fields.to(self.device)
    ...
    def get_CCM_variable(self, cur_path_vp):
        ...
        padding_mask = padding_mask | batch_gmap_visited_masks
    ...
    def _vp_feature_variable(self, obs):
        ...
        for i in range(self.envs.num_envs):
            ...
            lookahead_loc_fts.append(obs['pano_angle_fts'])
            ...
            batch_lookahead_loc_fts.append(torch.cat(lookahead_loc_fts, dim=0))
        ...
        batch_lookahead_loc_fts = pad_tensors_wgrad(batch_lookahead_loc_fts).cuda()

        return {
            'rgb_fts': batch_rgb_fts, 'dep_fts': batch_dep_fts, 'loc_fts': batch_loc_fts,
            'nav_types': batch_nav_types, 'view_lens': batch_view_lens, 'lookahead_loc_fts': batch_lookahead_loc_fts
        }
    ...
    def _nav_gmap_variable(self, cur_vp, cur_pos, cur_ori):
        ...
        for i, gmap in enumerate(self.gmaps):
            ...
            gmap_cand_pos = gmap.get_cand_pos(cur_vp[i], cur_pos[i], cur_ori[i], gmap_vp_ids)
            batch_gmap_cand_pos.append(gmap_cand_pos)
        ...
        return {
            ...
            'no_vp_left': batch_no_vp_left, 'batch_gmap_cand_pos': batch_gmap_cand_pos
        }
    ...
    def _train_interval(self, interval, ml_weight, sample_ratio):
        ...
        for idx in pbar:
            ...
            for name, parms in enumerate(self.policy.net.parameters()):
                if parms.grad != None and torch.any(torch.isnan(parms.grad)):
                    parms.grad[torch.isnan(parms.grad)] = 0
                    print('NaN in gradients,{} {}'.format(name, parms.grad.shape))
            #torch.nn.utils.clip_grad_norm_(self.policy.net.parameters(), max_norm=5.)
    ...
    def inference(self):
        ...
        if self.world_size > 1:
            ...
            distr.init_process_group(backend='nccl', init_method='env://',timeout=datetime.timedelta(seconds=7200000))
            self.local_rank = distr.get_rank()
            # print(f"Cuurrent rank:{rank}")
            # print(f"Cuurrent device:{self.device}")
            self.device = self.config.TORCH_GPU_IDS[self.local_rank]
            torch.cuda.set_device(self.device)
            self.config.defrost()
            self.config.local_rank = self.local_rank
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        prev_ghost = [None] * self.envs.num_envs
        prev_ghost_emb = [None] * self.envs.num_envs

        policy_net = self.policy.net
        if hasattr(self.policy.net, 'module'):
            policy_net = self.policy.net.module

        self.feature_fields.reset(batch_size=self.envs.num_envs, mode='habitat') # Reset the feature fields
        ...
        for stepk in range(self.max_len):
            ...
            if stepk == 0:          
                ...
                if self.config.MODEL.task_type == 'r2r':
                    self.feature_fields.initialize_camera_setting(hfov=90., vfov=90.)
                    self.feature_fields.initialize_novel_view_setting(hfov=90., vfov=90.)
                elif self.config.MODEL.task_type == 'rxr':
                    self.feature_fields.initialize_camera_setting(hfov=79., vfov=79.)
                    self.feature_fields.initialize_novel_view_setting(hfov=79., vfov=79.)

                self.feature_fields.reset(self.envs.num_envs)
            ...
            for i in range(self.envs.num_envs):
                ...
                ghost_set = self.gmaps[i].update_graph(prev_vp[i], prev_ghost_emb[i], stepk+1,
            ...
            for ei in range(self.envs.num_envs):
            ...
            nav_inputs.update({
                ...
                'txt_masks': txt_masks,
                'lookahead_loc_fts': vp_inputs['lookahead_loc_fts']
            })
            batch_gmap_cand_pos = nav_inputs['batch_gmap_cand_pos']
            nav_inputs.pop('batch_gmap_cand_pos')

            no_vp_left = nav_inputs.pop('no_vp_left')
            nav_outs = self.policy.net(**nav_inputs)
            nav_logits = nav_outs['global_logits']
            nav_probs = F.softmax(nav_logits, dim=-1)
            for i, gmap in enumerate(self.gmaps):
            ...
            batch_selected_cand_ids, batch_cand_position, batch_cand_heading, batch_cand_dist = [], [], [], []
            for batch_id in range(len(batch_gmap_cand_pos)):
                cand_dist = torch.tensor(batch_gmap_cand_pos[batch_id][2])
                _, cand_ids = torch.topk(nav_probs[batch_id],min(selected_cand_num,len(cand_dist)),largest=True)
                #_, cand_ids = torch.topk(cand_dist,min(selected_cand_num,len(cand_dist)),largest=False)
                cand_ids = cand_ids.cpu()
                if len(cand_ids) == selected_cand_num:
                    batch_cand_position.append(batch_gmap_cand_pos[batch_id][0][cand_ids])
                    batch_cand_heading.append(batch_gmap_cand_pos[batch_id][1][cand_ids])
                    batch_selected_cand_ids.append(cand_ids.unsqueeze(0))
                else:
                    #try:
                        batch_cand_position.append(np.concatenate([batch_gmap_cand_pos[batch_id][0][cand_ids]]+[batch_gmap_cand_pos[batch_id][0][0:1]]*(selected_cand_num-len(cand_ids)),axis=0))
                        batch_cand_heading.append(np.concatenate([batch_gmap_cand_pos[batch_id][1][cand_ids]]+[batch_gmap_cand_pos[batch_id][1][0:1]]*(selected_cand_num-len(cand_ids)),axis=0))
                        batch_selected_cand_ids.append(torch.cat([cand_ids]+[torch.tensor([0])]*(selected_cand_num-len(cand_ids)),dim=0).unsqueeze(0))
                        print("Not enough cand positions found...")
                    #except:
                    #    batch_cand_position.append([np.array([0.,0.,0.])]*selected_cand_num)
                    #    batch_cand_heading.append([np.array([0.])]*selected_cand_num)
                    #    batch_selected_cand_ids.append(torch.cat([torch.tensor([-1])]*selected_cand_num,dim=0).unsqueeze(0))
                    #    print("No cand positions found...")

            batch_cand_panorama_fts = []
            with torch.no_grad():
                for cand_index in range(selected_cand_num):
                    cand_panorama_fts = self.feature_fields.run_panorama_encode([cand_position[cand_index] for cand_position in batch_cand_position], [cand_heading[cand_index] for cand_heading in batch_cand_heading], panorama_encode=False)
                    batch_cand_panorama_fts.append(cand_panorama_fts.unsqueeze(0))

            #batch_bev_fts, _, _ = self.feature_fields.run_bev_encode(batch_positions,batch_headings,visualization=False)
            batch_cand_panorama_fts = torch.cat(batch_cand_panorama_fts,dim=0).to(self.device)
            batch_cand_panorama_fts = batch_cand_panorama_fts.permute(1,0,2,3) # !!!!!!!!!!!!!!!!

            batch_selected_cand_ids = torch.cat(batch_selected_cand_ids,dim=0).to(self.device)
            nav_inputs['cand_panorama_fts'] = batch_cand_panorama_fts
            nav_inputs['selected_cand_ids'] = batch_selected_cand_ids
            #nav_inputs['batch_bev_fts'] = batch_bev_fts

            nav_inputs.update({
                'mode': 'lookahead_navigation'
            })
            lookahead_logits = self.policy.net(**nav_inputs)
            org_nav_logits = nav_outs['global_logits_org']
            gmap_logits = org_nav_logits#.detach().clone()

            batch_selected_cand_ids = batch_selected_cand_ids.to(org_nav_logits.device)
            for batch_id in range(org_nav_logits.shape[0]):
                gmap_logits[batch_id][batch_selected_cand_ids[batch_id]] = lookahead_logits[batch_id]
            ...
            CCM_inputs.update({
                    ...
                    'gmap_logits': gmap_logits,
                    'txt_embeds': txt_embeds,
                    'txt_masks': ~txt_masks,
                })
            path_logits, global_logits, residual_logits, final_logits, weight = self.policy.net(**CCM_inputs)
            ...
            if mode == 'train':
                global_loss += F.cross_entropy(nav_logits, teacher_actions, reduction='sum', ignore_index=-100)
                global_loss += F.cross_entropy(global_logits, teacher_actions, reduction='sum', ignore_index=-100)
                loss += F.cross_entropy(final_logits, teacher_actions, reduction='sum', ignore_index=-100)
                path_loss += F.cross_entropy(path_logits, teacher_actions, reduction='sum', ignore_index=-100)
                residual_loss += F.cross_entropy(residual_logits, teacher_actions, reduction='sum', ignore_index=-100)
            ...
            if sum(dones) > 0:
                for i in reversed(list(range(self.envs.num_envs))):
                    if dones[i]:
                        ...
                        prev_vp.pop(i)
                        prev_ghost.pop(i)
                        prev_ghost_emb.pop(i)
                        self.feature_fields.pop(i)
        ...
        if mode == 'train':
            ...
            path_loss = ml_weight * path_loss / total_actions
            global_loss = ml_weight * global_loss / total_actions
            residual_loss = ml_weight * residual_loss / total_actions
            self.loss += loss + path_loss + global_loss + residual_loss
            self.logs['IL_loss'].append(loss.item())
            self.logs['Path_loss'].append(path_loss.item())
            self.logs['Global_loss'].append(global_loss.item())
            self.logs['Residual_loss'].append(residual_loss.item())
            self.logs["Total_loss"].append(self.loss.item())