...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _initialize_policy(
        ...
        # # 输出可训练参数名称
        # trainable_param_names = [name for name, param in self.policy.named_parameters() if param.requires_grad]
        # print("Trainable parameters:")
        # for name in trainable_param_names:
        #     print(f"  - {name}")
        # print(f"Total: {len(trainable_param_names)} trainable parameters")
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        for stepk in range(self.max_len):
            ...
            for i in range(self.envs.num_envs):
                ...
                ghose_set = self.gmaps[i].update_graph(prev_vp[i], prev_ghost[i], prev_ghost_emb[i], stepk+1,
            ...
            # fused_logits = path_logits
            # nav_probs = path_probs
        ...
        if mode == 'train':
            ...
            self.logs['IL_path_loss'].append(path_loss.item())
            self.logs['IL_global_loss'].append(global_loss.item())
            self.logs['IL_residual_loss'].append(residual_loss.item())
